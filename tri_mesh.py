...
class TriMesh:
    ...
    def complete(self):
        ...
        for iel in range(self.nelem):
            ...
            for iedg in range(3):
                ...
                self.normals[iel, iedg, :] = self.compute_normal(
                    edgvert, vertID, coords
                )
                self.edge_len[iel, iedg] = np.linalg.norm(
                    self.pt2xy[edgvert[0], :] - self.pt2xy[edgvert[1], :]
                )
    ...
    def compute_normal(self, edgvertID, trivertID, coord):
        ...
        other_vert = np.setdiff1d(
            np.union1d(trivertID, edgvertID), np.intersect1d(trivertID, edgvertID)
        )[0]
    ...
    def check_bc(self, bc, verbose=False):
        ...
        for key in key_set:
            ...
            if (bc.get(key)).get("markers").size > 0:
                ...
                if (bc.get(key)).get("markers").size != (bc.get(key)).get(
                    "values"
                ).size:
                    raise ValueError(
                        "marker and value arrays of different length for bc type = "
                        + key
                    )
    ...
    def sweep_order(self, omega, do_plot=False, verbose=False):
        ...
        while len(inc_edg) > 0:
            ...
            is_e2_incoming = (
                len(res[0]) > 0
            )  # whether e2 is ready to be an incoming edge
            res = np.where(inc_edg == edg3)
            is_e3_incoming = (
                len(res[0]) > 0
            )  # whether e3 is ready to be an incoming edge

            if (
                (is_e2_outgoing and is_e3_outgoing)  # both e2 and e3 are outgoing edges
                or (
                    is_e2_outgoing and is_e3_incoming
                )  # e2=outgoing, e3=ready as an incoming
                or (
                    is_e3_outgoing and is_e2_incoming
                )  # e3=outgoing, e2=ready as an incoming
            ):
    ...
    ###----------------------------------------------------------------------
    def sweep_order_unc(
        self, src_pt_in=[], near_src_region=False, do_plot=False, verbose=False
    ):
