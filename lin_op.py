    ...
    def build_diffusion_op_per_attribute(self):
        ...
        for imat in range(self.mesh.nattr):
            ...
            M = scipy.sparse.csr_matrix(
                (entries_m, (rows, cols)), shape=(self.mesh.npts, self.mesh.npts)
            )
            K = scipy.sparse.csr_matrix(
                (entries_k, (rows, cols)), shape=(self.mesh.npts, self.mesh.npts)
            )
            self.M.append(M)
            self.K.append(K)
            self.q.append(q)
    ...
    def compute_reduced_operators(self, ur, bc):
        ...
        for i in range(self.mesh.nattr):
            for j in range(self.mesh.nattr):  # sum on j starts at 0
                self.MK.append(
                    (ur.T @ self.M[i] @ self.K[j] @ ur) * 2
                )  # here, the factor 2 is b/c (a+b)^2 = a^2+2ab+b^2
        ...
        if key in bc.keys():
            ...
            for i in range(self.mesh.nattr):
                self.KArob.append(
                    (ur.T @ self.K[i] @ self.Arob @ ur) * 2
                )  # here, the factor 2 is b/c (a+b)^2 = a^2+2ab+b^2
                self.MArob.append(
                    (ur.T @ self.M[i] @ self.Arob @ ur) * 2
                )  # here, the factor 2 is b/c (a+b)^2 = a^2+2ab+b^2
            # M/K to qrob
            ...
            for i in range(n_rob):
                for j in range(i, n_rob):  # sum on j starts at i
                    if i == j:
                        ...
                        factor = (
                            2  # because we only compute the triangular part (symmetry)
                        )
    ...
    def residual_indicator(self, qext, cdif, siga, bc, c):
        ...
        for i in range(self.mesh.nattr):
            ...
            for j in range(i, self.mesh.nattr):  # sum on j starts at i
                ...
                siga_j = siga[j]
                Arr += (
                    cdif_i * cdif_j * self.KK[counter]
                    + siga_i * siga_j * self.MM[counter]
                )
                counter += 1
        counter = 0
        for i in range(self.mesh.nattr):
        ...
        if key in bc.keys():
            ...
            for i in range(n_rob):
                ...
                for j in range(self.mesh.nattr):
                    ...
                    siga_j = siga[j]
                    A1r += Jinc_i * (
                        cdif_j * self.Kqrob[counter] + siga_j * self.Mqrob[counter]
                    )
                    counter += 1
            # Arob to q
            counter = 0
        ...
        if key in bc.keys():
            ...
            for i in range(n_neu):
                ...
                for j in range(self.mesh.nattr):
                    ...
                    siga_j = siga[j]
                    A1r += Jneu_i * (
                        cdif_j * self.Kqneu[counter] + siga_j * self.Mqneu[counter]
                    )
                    counter += 1
            # Arob to qneu
            counter = 0
        ...
        if errorIndicator < 0:
            if abs(errorIndicator) > 1e-10:
                raise ValueError(
                    "==> error indicator can only be <0 within roundoff :",
                    errorIndicator,
                )
            else:
