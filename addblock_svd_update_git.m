function [Up,Sp,Vp] = addblock_svd_update_git( U, S, V, A, force_orth, do_test )
...
%
  ...
  if ~do_test
      ...
      [tUp,tSp,tVp] = svd( K, 'econ');
      % uncomment this if we want to keep the initial size and not grow
      %       [tUp_,tSp_,tVp_] = svd( K, 'econ');
      %       tUp = tUp_(:,1:current_rank);
      %       tSp = tSp_(1:current_rank,1:current_rank);
      %       tVp = tVp_(:,1:current_rank);
  end
  %
